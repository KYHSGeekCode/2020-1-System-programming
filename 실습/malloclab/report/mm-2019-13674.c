 ...
 * 
 * In this approach, a block is allocated by searching for a block which is free or
 * incrementing the brk pointer.  A block has a size_t sized header. Blocks are coalesced or reused.
 * Realloc is implemented effecive by coalescing or directly using mm_malloc and mm_free.
 *
 *
 *
 *
...
/* 
 * mm_malloc - Allocate a block by searching the first fit from the nodes list, Then split the left block to allocatables. If not found, increase the brk pointer.
...
/*
 * mm_free - Freeing a block marks the block as free, and tries coalescing
 */
...
/*
 * mm_realloc - Implemented effectively using coalescing or  in terms of mm_malloc and mm_free
 */
		...
		if(memcmp(iterator, FTRP(iterator), SIZE_T_SIZE)) {
			...
			for(int i=0;i<SIZE_T_SIZE; i++) {
				if(iterator[i] != FTRP(iterator)[i]) {
					printf("node[%d]:%x footer[%d]:%x\n", i, (int)iterator[i] , i,  (int)FTRP(iterator)[i]);
				}
			}
			return 0;
		}
